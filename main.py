...
inputs = mpc_control(init_x,init_y,interp_func,0,0,T,1000*np.identity(1),np.identity(1),dt)
...
for i in range(T-1):
    ...
    ol_x[i+1] = new_ol_x
    ol_y[i+1] = new_ol_y
    cost += -interp_func(new_ol_x,new_ol_y)*1000
...
ax3.plot(ol_x, ol_y, linewidth=1, color='r',alpha=1, label="path")
...
ax5.set_title('Value of costfunction')
